...
static int dglnt_dynclk_probe(struct platform_device *pdev)
{
	...
	printk("Digilent Dynamic Clock Probe Started\n");
	...
	of_property_read_string(pdev->dev.of_node, "clock-output-names",
		&clk_name);
	printk("digilent dynclock name: %s\n", clk_name);
	init.name = clk_name;
   ...
   dglnt_dynclk->freq = 0;
	...
	printk("Digilent Dynamic Clock Probed\n");
...
static int __init dglnt_dynclk_init(void) {
	int ret;
	ret = platform_driver_register(&dglnt_dynclk_driver);
	return ret;
}
static void __exit dglnt_dynclk_exit(void) {
	
	platform_driver_unregister(&dglnt_dynclk_driver);
	return;
}
postcore_initcall(dglnt_dynclk_init);
module_exit(dglnt_dynclk_exit);
